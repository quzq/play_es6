...
export const stretchSize = (
  ...
  if (parentHeight > height)
    return { width: parentWidth, height, base: 'width' };
  // height基準にストレッチ
...
export const getAverageWithoutOutlier = (values) => {
  ...
  const forAverage = dists
    .filter((v) => v.idx !== outlier.idx)
    .map((v) => v.value);
...
export const getStartDateInThisWeek = (weekStartDay, today) => {
  ...
  return dayjs(today)
    .add(-((thisWeekDay + 7 - weekStartDay) % 7), 'day')
    .toDate();
};
...
export const sortFloorNo = (floors) => {
  ...
  const floorNos = _.uniqWith(
    floors.map((i) => ({
      code: i.floorNo,
      order: parseInt(i.floorNo.replace('B', '-')),
    })),
    _.isEqual
  ).sort(sortDesc);
  // 地上階は昇順で選択、地下階は降順で選択
  ...
  console.log(
    'filter',
    floorNos.filter((i) => i.order > 0)
  );
  console.log('asc', floorNos.filter((i) => i.order > 0).sort(sortAsc));
  console.log(
    'first',
    _.first(floorNos.filter((i) => i.order > 0).sort(sortAsc)) || 'what?'
  );

  return (
    _.first(floorNos.filter((i) => i.order > 0).sort(sortAsc)) ||
    _.first(floorNos.filter((i) => i.order < 0).sort(sortDesc))
  ).code;
};
...
export const convertPoints = (s) => {
  ...
  const points = [...Array(numbers.length / 2).keys()].map(
    (i) => `${numbers[i * 2]},${numbers[i * 2 + 1]} `
  );
...
export const groupChildCode = (arr) => {
  return arr.reduce((sum, curr) => {
    ...
    if (!lastCorp || lastCorp.corp_cd !== curr.corp_cd) {
      return [
        ...sum,
        { corp_cd: curr.corp_cd, store_cds: curr.store_cd && [curr.store_cd] },
      ];
    } else {
